...
// CODE HERE
const testfunc = (para1, para2) => { return (`The first parameter is ${para1}. The second paramenter is ${para2}.`)} 
...
function conca(firstName,lastName) {
    return (`${firstName} ${lastName}`)
}
function greeting(firstName, lastName, myCallback) {
   return myCallback(firstName,lastName);
}
console.log('Hello, my full name is '+ greeting('Nhu','Le',conca))
...
function pricesPlusTax(arr, myCallback) {
...
// CODE HERE
console.log(pricesPlusTax(prices, newprice))
...
function multiplyingFactory(num1) {
    return (function multiplycheck(num2) {
        if (num1 >= 5) {
            return (num1*num2)
        } else { return ('Cannot multiply: the first number is smaller than 5.')}
    } )
}
...
// CODE HERE
var timesFour = multiplyingFactory(3)
...
*/
var timesFour = multiplyingFactory(5)
console.log(timesFour(4))